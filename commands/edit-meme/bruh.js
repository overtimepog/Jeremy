...
module.exports = class BruhCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'bruh',
			group: 'edit-meme',
	...
	async run(msg, { image }) {
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'bruh.png'));
			...
			return msg.say({ files: [{ attachment, name: 'bruh.png' }] });
		} catch (err) {
